...
def get_dataloaders(dataset, batch, batch_unsup, dataroot):
    ...
    if dataset in ['cifar10', 'cifar100','mnist']:
        if dataset == 'cifar10':
            ...
            total_trainset = torchvision.datasets.MNIST(root=dataroot, train=True, download=True, transform=transform_train)
            unsup_trainset = torchvision.datasets.MNIST(root=dataroot, train=True, download=True, transform=None)
